...
class InstallationBackend:
    ...
    def _run(self, cmd, check=True, capture=False, chroot=False):
        ...
        try:
            ...
            if capture:
                return res
            if res.stdout and len(res.stdout) < 300:
                self._log(f"OUT: {res.stdout.strip()}")
            if res.stderr:
                if "warning" in res.stderr.lower() or "information" in res.stderr.lower():
                    pass
                else:
                    self._log(f"LOG: {res.stderr.strip()}")
            return res
        except subprocess.CalledProcessError as e:
            self._log(f"!! FEHLER (Exit {e.returncode}): {e}")
            if e.stderr:
                self._log(f"DETAILS: {e.stderr.strip()}")
            if check:
                raise
            return e
    ...
    def _part_freespace(self, plan):
        ...
        if not res or not res.stdout:
            raise Exception("Partitionstabelle nicht lesbar")
        ...
        if max_size < 10000:
            raise Exception("Zu wenig freier Speicher (<10GB).")
    ...
    def _create_layout(self, dev, uefi, start, end, plan):
        ...
        while True:
            ...
            if os.path.basename(pn) not in out and not os.path.exists(pn):
                break
            idx += 1

        try:
            s_val = float(start.replace("MiB", ""))
        except:
            s_val = 1.0
        ...
        # 3. Root & Home
        try:
            total_end = 999999.0 if "%" in end else float(end.replace("MiB", ""))
        except:
            total_end = 999999.0
        ...
        if fs == "btrfs":
            ...
            if root:
                self._btrfs_subvols(root, parts, plan.get("use_home"))

        return parts

    def _devp(self, d, i):
        return f"{d}p{i}" if "nvme" in d or "mmcblk" in d else f"{d}{i}"

    def _mkfs(self, d, fs):
        self._log(f"Format {d} ({fs})")
        if fs == "vfat":
            self._run(["mkfs.vfat", "-F32", d])
        elif fs == "swap":
            self._run(["mkswap", d])
        elif fs == "btrfs":
            self._run(["mkfs.btrfs", "-f", d])
        elif fs == "xfs":
            self._run(["mkfs.xfs", "-f", d])
        else:
            self._run(["mkfs.ext4", "-F", d])
    ...
    def _btrfs_subvols(self, dev, parts, has_home):
        ...
        if not has_home:
            subs.append("@home")
        for s in subs:
            if not os.path.exists(f"{m}/{s}"):
                self._run(["btrfs", "subvolume", "create", f"{m}/{s}"])
        self._run(["umount", m])
        parts[dev]["subvol"] = "@"
        if not has_home:
            parts["VIRTUAL_HOME"] = {"device": dev, "mountpoint": "/home", "subvol": "@home"}
    ...
    def _mount_filesystems(self, plan):
        ...
        # 3. Others
        for d, i in parts.items():
            if d == "VIRTUAL_HOME" or i["mountpoint"] in ["/", "[SWAP]"]:
                continue
    ...
    def _copy_live_filesystem(self):
        ...
        if shutil.which("rsync"):
            ...
            if res.returncode not in [0, 23, 24]:
                raise Exception(f"rsync failed: {res.returncode}")
        else:
        ...
        with open(f"{self.target_root}/etc/fstab", "w") as f:
            ...
            if res and res.stdout:
                for l in res.stdout.splitlines():
                    ...
                    if t == self.target_root:
                        t = "/"
                    else:
                        t = t.replace(self.target_root, "")
    ...
    def _configure_basics(self, plan):
        with open(f"{self.target_root}/etc/hostname", "w") as f:
            f.write(plan["user"]["hostname"])
        with open(f"{self.target_root}/etc/locale.conf", "w") as f:
            f.write(f"LANG={plan['language']}\n")
        with open(f"{self.target_root}/etc/rc.conf", "a") as f:
            f.write(f"KEYMAP={plan['keyboard']}\n")
        if os.path.exists(f"{self.target_root}/etc/localtime"):
            os.remove(f"{self.target_root}/etc/localtime")
    ...
    def _configure_user(self, plan):
        ...
        # --- WICHTIG: root darf NICHT als normaler Benutzer angelegt werden ---
        if username == "root":
            raise Exception("Der Benutzername 'root' ist nicht erlaubt. Bitte einen anderen Namen wählen.")

        self._enter_chroot()
        try:
            # Live-User entfernen (falls vorhanden)
            ...
            # Benutzer anlegen + Gruppen (inkl. lpadmin + scanner)
            groups = "wheel,users,audio,video,network,input,storage,lpadmin,scanner"
            self._run(["useradd", "-m", "-s", "/bin/bash", "-G", groups, username], chroot=True)

            # Benutzer-Passwort setzen
            p = subprocess.Popen(["chroot", self.target_root, "passwd", username], stdin=subprocess.PIPE, text=True)
            p.communicate(f"{u['password']}\n{u['password']}\n")

            # sudo für wheel
            with open(f"{self.target_root}/etc/sudoers.d/wheel", "w") as f:
    ...
    def _finalize(self):
        self._enter_chroot()
        try:
            self._run(["xbps-reconfigure", "-fa"], chroot=True)
        finally:
            self._leave_chroot()
    ...
    def _enter_chroot(self):
        ...
        if os.path.exists("/sys/firmware/efi/efivars"):
            ...
            if not os.path.ismount(t):
                self._run(["mount", "--bind", "/sys/firmware/efi/efivars", t], check=False)
        try:
            shutil.copy("/etc/resolv.conf", f"{self.target_root}/etc/resolv.conf")
        except:
            pass

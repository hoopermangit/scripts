...
class UserConfigView(Gtk.Box):
    ...
    def validate_user_input(self):
        ...
        if not user or not re.match(r"^[a-z_][a-z0-9_-]*$", user):
            raise ValueError(f"Nombre de usuario inválido: '{user}'. Por favor, utilice sólo letras minúsculas y números..")

        # --- WICHTIG: root sperren ---
        if user == "root":
            raise ValueError("El nombre de usuario 'root' no está permitido. Elija otro nombre..")

        if not host:
            raise ValueError("Hostname darf nicht leer sein.")

        if self.entry_pw.get_text() != self.entry_pw2.get_text():
            raise ValueError("Las contraseñas de usuario no coinciden..")

        if not self.entry_pw.get_text():
            raise ValueError("Crea una contraseña de usuario..")

        if self.entry_root.get_text() != self.entry_root2.get_text():
            raise ValueError("Las contraseñas de root no coinciden..")

        if not self.entry_root.get_text():
            raise ValueError("Crea una contraseña para root..")

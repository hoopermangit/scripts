...
class SummaryView(Gtk.Box):
    ...
    def update_summary_data(self, p):
        ...
        d = [
            (T("sum_dev"), p["device"]),       # Disco duro de destino
            (T("sum_fs"), p["filesystem"]),    # Sistema de archivos
            (T("sum_mode"), mode_text),        # Moduo
            (T("part_home"), home_text),       # Particion Home
            (T("sum_host"), p["user"]["hostname"]), # Hostname
            (T("sum_user"), p["user"]["name"]),     # Usuario
        ]
